...
def Navigation():
    ...
    try:
        wait(Driver, 50).until(EC.presence_of_element_located((By.XPATH, '//h1[contains(text(), "Discover the best food & drinks in")]')))
    ...
    try:
        PopularLocalities = Driver.find_element(By.XPATH, '//p[contains(text(), "Popular localities in and around")]')
        Driver.execute_script("arguments[0].scrollIntoView();", PopularLocalities)
        print("Scroll down successfully upto Popular localities in and around")
        time.sleep(5)
    except:
        print("Unable to Scroll down successfully upto Popular localities in and around")
        pass
...
def Scraped():
    ...
    for location in AreaList:
        # -- Click on current location link --
        try:
            print("\n+++++++++++++++++++ Area Name is: " + str(location) + " +++++++++++++++++++\n")
            wait(Driver, 20).until(EC.element_to_be_clickable((By.XPATH, '//h5[contains(text(), "' + str(location) + '")]/../../..'))).click()
            print(location + " Location selected successfully")
            time.sleep(2)
        except:
            print("!!! Unable to Select location : " + location)
            
        # -- Check the location data page is loaded or not --
        try:
            wait(Driver, 15).until(EC.presence_of_element_located((By.XPATH, '//h2[contains(text(), "Collections")]')))
            print("Location data page is loaded successully")
            time.sleep(2)
            DataAvailable = False
        except:
            print("!!! Location data page is not loaded")
        
        DataAvailable = True
        # -- Check Sorry, no results found is showing or not --
        try:
            wait(Driver, 5).until(EC.presence_of_element_located((By.XPATH, '//h3[contains(text(), "Sorry, no results found")]')))
            ...
            for PlaceCount in range(1,11):
                try:
                    ...
                    try:
                        ...
                        FileResult = open("Zomato_Info.csv", 'a',encoding='utf-8')
                        FileResult.write(str(location) + '|' + str(PlaceName) + '|' +  str(Price) + '|' + str(Star) + '|' + str(MenuExample) + '\n')
                        FileResult.close()
                    except:
                        print("!!! Unable to write data in csv")
...
def Runbot():
    ...
    FileResult = open("Zomato_Info.csv", "w")
